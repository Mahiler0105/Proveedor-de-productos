...
const locals: Locals = {ini: [{id: 1,icon: "plus",type: "material-community"}], all: []}
const iconsavailable =[{icon: "home"},{icon: "drive-eta"},{icon: "headset"},{icon: "laptop"},
{icon: "beach-access"},{icon: "cloud"},{icon: "favorite"},{icon: "free-breakfast"}]
...
const Seticons =(props)=>{    
    ...
    return(
        <TouchableOpacity style={{marginRight:(ind+1)%len==0?0:5}} onPress={()=>{setnum(ind)}}>            
            ...
            padding: 5}}>
                <Icon name={name} size={20} color={num==ind?color:'rgba(189, 187, 178, 0.3)'} />
            </View> 
        </TouchableOpacity>
    )
}
const Children = (props)=>{    
    ...
    const addStore = async () => {
        ...
        await db.collection("Tienda").doc().set({ 
    ...
    return(
        <>
            ...
            <Input placeholder={edit?data[ind].cont:"Nombre del local"} onChange={(e) => setNombreLocal(e.nativeEvent.text)}
                leftIcon={<Icon name='store' type='material' size={24} color={color} />}
                rightIcon={<Icon iconStyle={{opacity: 0}} name='error' type='material' size={24} color={"red"} />}></Input>
            <Input placeholder={edit?data[ind].cont:"Persona de contacto"} onChange={(e) => setPersonaContacto(e.nativeEvent.text)}
                leftIcon={<Icon name='person' type='material' size={24} color={color} />} 
                rightIcon={<Icon iconStyle={{opacity: 0}} name='error' type='material' size={24} color={"red"} />}></Input>
            <Input placeholder={edit?data[ind].cell:"Celular"} onChange={(e) => setCelular(e.nativeEvent.text)}
                leftIcon={<Icon name='phone' type='material' size={24} color={color} />} 
                rightIcon={<Icon iconStyle={{opacity: 0}} name='error' type='material' size={24} color={"red"} />}></Input>
            <Input placeholder={edit?data[ind].desc:"Direccion completa"} onChange={(e) => setDireccion(e.nativeEvent.text)}
                leftIcon={<Icon name='location-city' type='material' size={24} color={color} />} 
                rightIcon={<Icon iconStyle={{opacity: 0}} name='error' type='material' size={24} color={"red"} />}></Input>
            ...
            <View style={{width: '100%', justifyContent: 'center', alignItems: 'center', flexDirection: 'row', marginTop:10, marginBottom:10 }}>
                ...
                <Text style={{ fontSize: 15, fontWeight: 'bold', marginRight: 10}}>Mapa</Text>
                <TouchableOpacity onPress={()=>{m(true);viewall(false);}}>
                    <Text style={{color: '#245c9c'}}>{coor.latitude==0?"Ubique en el mapa":coor.latitude+"º "+coor.longitude+"º"}</Text>
                </TouchableOpacity>
            </View>
            <View style={{width: '100%', justifyContent: 'center', alignItems: 'center', flexDirection: 'row', marginTop:10, marginBottom:10 }}>
...
const Storefill = (props) => {
    ...
    return (
        <TouchableOpacity style={(ind+1)%2==0 ? {marginBottom:5,marginRight:5}:{marginBottom:5,marginRight:0}} onPress={()=>{v(true);e(true);s(ind);c(locals.all[ind].dire)}}>
...
const MainStore = (props) =>{  
    ...
    useEffect(()=>{
        ...
        const loadStores = async () => {
            locals.all = []
            await db.collection("Tienda").where("idpr", "==", cuser).orderBy('crat').get().then((response) => {   
    ...
    return (  
        ...
        <View style={styles.super}>
            ...
            <TouchableOpacity style={{marginRight: 5, marginBottom:5,borderRadius: 20}} onPress={()=>{setVisible(true);setEdit(false);setCoordinates(empty)}}>
            ...
            {locals.all.map((store, key) => (
                <Storefill key={key} ind={key} store={store} e={setEdit} v={setVisible} s={setInd} c={setCoordinates}/>
            ))}             
            </View>
            {locals.all.length>1?(
            <View style={{width: '100%', justifyContent: 'center', alignItems: 'center', flexDirection: 'row', marginTop:10, marginBottom:10 }}>
