    ...
    <>
      ...
      >
        <Text style={{ fontSize: 24 }}>
          {all ? title : "Ubique el punto exacto de su tienda"}
        </Text>
      </View>
      {children}
      <View
      ...
      >
        <MapView
          ...
          initialRegion={all ? initial : retrieved}
          onPress={(e) => {
        ...
        >
          {all ? (
            places.map((store, key) => (
              <Marker
                key={key.toString()}
                coordinate={store.dire}
                title={store.name}
                description={store.cont}
              />
            ))
          ) : (
            <>
              <Marker
                ...
                description={
                  places.length == 0 ? "Nueva descripcion" : places[index].desc
                }
              />
            </>
          )}
